...
def annealing(env, num_iterations=1000, episodes_per_iteration=20, min_spread=0.1, max_spread=0.25, starting_temperature=10000, cooling_rate=0.999):
    history = []
    best_reward = 0
    ...
    for iteration in range(num_iterations):
        noise = np.random.normal(0, spread, size=best_weights.shape)
        ...
        formatted_weights = np.array2string(current_weights, precision=2, separator=',', suppress_small=True)
        print(f"\tIteration {iteration}: Average Reward = {current_reward}\t{formatted_weights}\tTemperature = {temperature:.2f}\tSpread = {spread:.2f}")
